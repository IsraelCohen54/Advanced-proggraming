 ...
 */
template <typename T>
bool safeVector(vector<T> veccush,int index) {

	if (veccush.empty()){
		return false;
	}
	int vec__size=veccush.size();
	if ((index<=vec__size)&&(index>=0)) {
		return true;
	}
return false;
}
SimpleAnomalyDetector::SimpleAnomalyDetector() {
//std::vector<std::vector<correlatedFeatures>> m_flight_data_normal;
}
SimpleAnomalyDetector::~SimpleAnomalyDetector() {
	// TODO Auto-generated destructor stub
}
vector<string> split_det (const string &s, char delim) {
...
void array_creation_from_vec(const TimeSeries& ts, int index_table, float* array_column) {
...
void SimpleAnomalyDetector::learnNormal(const TimeSeries& ts){
	//cout<<"simple anomaly cpp start work ok"<<endl;
	...
	for (int i = 0; i < no_offeatures - 1; i++) {
		array_creation_from_vec(ts,  i, column_arr1);
		
		for (int j = i+1; j < no_offeatures; j++) {
			array_creation_from_vec(ts,  j, column_arr2);
			temp_corelation=pearson(column_arr1,column_arr2,vec_size_to_pearson);
			if((abs(temp_corelation)>abs(highest_coreltaion)) && (abs(temp_corelation) >= 0.9)) {
				array_creation_from_vec(ts, i, current1);
				array_creation_from_vec(ts, j, current2);
	...
	,.corrlation=highest_coreltaion,.lin_reg=the_lnr_f,.threshold=1.2f*max_dev,.circle_mid_point=Point(0.f,0.f)};
